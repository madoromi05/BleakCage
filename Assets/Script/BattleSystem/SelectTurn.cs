...
public class SelectTurn : MonoBehaviour
{
    ...
    /// <summary>
    /// ÀÛ‚Ì‘I‘ğˆ—‚ğs‚¤ƒRƒ‹[ƒ`ƒ“
    /// </summary>
    private IEnumerator SelectionProcessCoroutine()
    {
        Debug.Log($"‘I‘ğƒ^[ƒ“‚ÌƒvƒŒƒCƒ„[”: {currentParty.Count}");
        // ƒvƒŒƒCƒ„[l”•ª‚Ì‘I‘ğƒ‹[ƒv
        for (int pIndex = 0; pIndex < currentParty.Count; pIndex++)
        {
            ...
            playerUIs[pIndex].SetHighlight(new Color(0.5f, 0.8f, 1f));  // ’N‚Ìƒ^[ƒ“‚©•ª‚©‚è‚â‚·‚­‚·‚é

            // —Dæ‡ˆÊ3‚Â•ª‚Ì‘I‘ğƒ‹[ƒv
            for (int priority = 1; priority <= currentParty.Count; priority++)
            {
                Debug.Log($"Player {pIndex + 1} ‚Ì —Dæ‡ˆÊ {priority} ‚ğ‘I‘ğ‚µ‚Ä‚­‚¾‚³‚¢B(–îˆóƒL[‚Å‘I‘ğAEnterƒL[‚ÅŒˆ’è)");

                // ¶‚«‚Ä‚¢‚é“G‚ÌƒŠƒXƒg‚ğ–ˆ‰ñæ“¾‚·‚é
                var livingEnemies = currentEnemies.Where(e => e.EnemyHP > 0).ToList();
                if (livingEnemies.Count == 0)
                {
                    Debug.LogWarning("‘I‘ğ‰Â”\‚È“G‚ª‚¢‚Ü‚¹‚ñB");
                    break; // ‚±‚ÌƒvƒŒƒCƒ„[‚Ì‘I‘ğ‚ğ’†’f
                }
                ...
                if (targetUI != null)
                {
                    targetUI.SetHighlight(new Color(1f, 0.5f, 0.5f)); // ÔF
                }

                // ‘I‘ğ‚ªŠm’è‚·‚é‚Ü‚Å–³ŒÀƒ‹[ƒv
                while (true)
                {
                    yield return null;
                    ...
                    if (selectionChanged)
                    {
                        // ‘O‚Ìƒ^[ƒQƒbƒg‚ÌƒnƒCƒ‰ƒCƒg‚ğƒŠƒZƒbƒg
                        ...
                        if (currentUI != null)
                        {
                            currentUI.SetHighlight(new Color(1f, 0.5f, 0.5f)); // ÔF
                        }
...
<<<<<<< HEAD
                        ...
                        {
                            Debug.Log("‚»‚Ì“G‚ÍŠù‚É‘I‘ğÏ‚İ‚Å‚·B•Ê‚Ì“G‚ğ‘I‘ğ‚µ‚Ä‚­‚¾‚³‚¢B");
